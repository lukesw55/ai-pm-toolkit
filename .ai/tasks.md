...
- [x] Address the owner's review of PR #23 (head `f5523d2`): the grader reads every assertion through a soft-wrap normaliser and every good fixture re-wrapped at 72 columns stays in band; `validate-skill-repo-health` accepts the honest zero on the strength of the checks and needs one remedy per real failure, with zero- and one-finding controls. Second round (2026-09-15): the normaliser decides each break from the two lines it joins, so an unwrapped paragraph elsewhere no longer blocks the rejoin, enumerated labels stay boundaries, and each failure needs its own remedy before the next failure or a remedy that names both.
