...
Rule: if a file needs banners to be readable, split the file or improve names. Note: banner lines (5+ `=` chars) on newly added lines are hard-blocked by `.claude/hooks/anti-slop-gate.sh` outside of vendored paths.
