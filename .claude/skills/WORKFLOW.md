...
Transversal skills (`pm-transversal-stakeholder`, `pm-transversal-docs`, `pm-transversal-analysis`, `data-science-analyst`) apply at **every stage**: use them whenever cross-function alignment, Confluence/Jira publication, quali+quant synthesis, or technical data-work (audit a CSV export, validate SQL, check leakage, A/B-test analysis, baseline ML) is needed. The first three are the PM lens; `data-science-analyst` is the technical lens under them. `pm-transversal-analysis` decides what the data *means for product*; `data-science-analyst` decides whether *the analysis itself* is sound. They chain. In Discover, `data-science-analyst` is fine for file-level audit, but ML/forecasting and instrumentation queries stay out until Define and Develop.
...
| Reply-time runtime gate | `.claude/hooks/scope-bloat-gate.sh` (Stop hook) | auto-blocks once per turn: em-dash density, label-colon runs, headers on short prompts, dual-question close, scope bloat |
...
`.claude/settings.json` defines a `UserPromptSubmit` hook that injects the current stage into every Claude Code turn, making the agent stage-aware automatically. See `scripts/stage_context.py` for the implementation. Removing the hook is safe: the skills and agents still work, you just lose automatic stage-surfacing and read `active-context.md` manually.
