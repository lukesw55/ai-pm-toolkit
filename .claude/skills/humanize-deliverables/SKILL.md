...
2. **Pull the catalogue.** Read `.claude/skills/humanizer/SKILL.md` (or the relevant section if you've already loaded it this session). It contains the full 29-pattern list with before/after examples.
...
The publish/send MCP tools listed in step 7 are **hard-gated** by `.claude/hooks/humanize-gate.sh` via a `PreToolUse` hook in `.claude/settings.local.json`. The hook computes sha256 over the longest string in `tool_input` (the prose body, in practice) and blocks the call unless `.claude/.humanized/<hash>.flag` exists.
