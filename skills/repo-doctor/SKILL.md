...
6. **Skill frontmatter** — every `skills/*/SKILL.md` (excluding `external/`) has a YAML frontmatter block with `name` and `description`.
...
8. **Mirror parity** — `python scripts/sync_skills.py --check` exits 0; `skills/`, `.claude/skills/`, and `.agents/skills/` are byte-identical (excluding `workspace/`).
