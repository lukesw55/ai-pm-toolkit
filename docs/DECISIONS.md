...
| Human labels are tracked and bound to the output hash | Verdicts live in `docs/benchmarks/<iteration>/labels.jsonl`, appended only, one per labeler; the grader reports disagreement per skill and overall and flags drift above 10%, at which point assertions are recalibrated before pass rates are trusted. The pilot itself runs on the owner's machine with both CLIs authenticated. | References batch, authorized 2026-09-10 |
