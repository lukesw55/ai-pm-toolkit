...
`scripts/test_grade_evals.py` runs hand-written outputs through the real assertion blocks. The pedagogical fixtures at the top of the file pin exact bands, and every pair in `scripts/fixtures/adversarial_outputs.json` must score its good text in 0.80 to 1.00 and its bad text at 0.30 or below, with a gap of at least 0.50. Every graded eval (negative-control and adversarial) carries a strict pair: a keyword-only reply that stays at or below 0.34 in five punctuation joins, a near miss that fails exactly one named assertion, and the block's own labels read back as a reply in the same five joins, also at or below 0.34. Coverage is derived from the manifests: a graded eval without a high fixture, a low fixture or a strict pair fails the suite. The contract the fixtures enforce is written in `docs/EVAL_PROTOCOL.md`.
