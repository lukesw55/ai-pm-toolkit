...
- README contract: the skill, blocking-hook, agent and eval counts in `README.md` are recomputed from the tree and must match, each stated exactly once as a digit (zero matches means the sentence was reworded and the check stopped protecting it); the scripts table has one row per file in `scripts/` and no row for a file that is gone; and the contents list matches the top-level headings in order. Adding a skill or a script is meant to fail here until the README says so; `CONTRIBUTING.md` says why. The counts are derived defensively: a manifest that does not parse, or whose shape `check_eval_coverage` is there to reject, is skipped for counting and takes the two manifest-derived comparisons with it, because this check runs first and a traceback here would hide the finding that check exists to produce.
