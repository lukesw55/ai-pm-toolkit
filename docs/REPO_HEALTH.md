...
- README contract: the skill, blocking-hook, agent and eval counts in `README.md` are recomputed from the tree and must match, each stated exactly once as a digit (zero matches means the sentence was reworded and the check stopped protecting it); the scripts table has one row per file in `scripts/` and no row for a file that is gone; and the contents list matches the top-level headings in order. Adding a skill or a script is meant to fail here until the README says so; `CONTRIBUTING.md` says why. The counts are derived defensively and from the canonical skill set: the manifests read are the ones `skills/*/SKILL.md` are expected to carry, so one under a directory that is not a skill never reaches the totals. A manifest that is missing, unreadable, not UTF-8 or not JSON, or whose shape or category `check_eval_coverage` is there to reject, is skipped for counting and takes the two manifest-derived comparisons with it, because this check runs first and a traceback here would hide the finding that check exists to produce.
...
`scripts/test_propose_eval_updates.py` covers the proposal pass: which labelled runs become a card and which deliberately do not (agreement in either direction, a split with no consolidated verdict, and any run nobody labelled), the precedence that makes an eval-defect handle outrank a false accept, a stale-rubric card that asks for a relabel rather than a correction, the fixture candidate as a replacement for one slot of the existing pair rather than a second object the coverage checks would not notice, the collision warning when that candidate is already another slot's value, the default that keeps the recorded output out of both tracked files and the `--embed-output` pass that writes it in, idempotence and the refusal to overwrite a proposal a human annotated, and the guarantee the whole script exists for: after a pass, `evals.json`, the assertion blocks and `scripts/fixtures/adversarial_outputs.json` are byte-identical and no grading record was written. Its `check` mode is covered too: the shape of a pasted candidate is judged before the suite is run, so a missing key or a duplicate object is a sentence rather than a traceback; then the band of every fixture the suite ran for that eval, the eight derived from the pair and any written in code beside them, the declared against the actual near-miss failure, the discrimination gap, the derived attacks and any fixture the change moved elsewhere are reported step by step, against the real tree for the green case and against a copy for the red ones.

`scripts/test_golden_set.py` runs the real `scripts/golden_set.py` against a throwaway project: the two files it creates from the tracked templates and its refusal to overwrite them, confinement through `scripts/context_paths.py` (a PII slug, a PII segment as the feature name, traversal on either side), CSV quoting for a reason that contains a comma, the refusal of a field that spans lines, an unknown handle as a warning where an unknown label is a refusal, the guard that reads and validates the whole sheet before appending so a hand-corrupted file is never half-written, and the checks the reference calls for: every row carries a reason, dates parse and are not in the future, duplicate locators warn, and a set whose only failure is synthetic does not satisfy the rule that one row is a failure the team has really seen.
