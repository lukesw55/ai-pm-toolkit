...
| `test_grade_evals.py` | fixtures for the grader's assertion blocks: good output has to score high, bad output low; every graded eval carries a strict pair whose keyword-only reply stays low in five punctuation joins, whose near miss fails exactly one named assertion and whose block fails its own labels read back as a reply |
